...
# example
#   QUINCE_DEPLOY_DIR      : deploy-quince-image-minimal-am335x-evm
...
cp ${DEPLOY_DIR_IMAGE}/${IMAGE_LINK_NAME}.wic.xz ${QUINCE_DEPLOY_DIR}
...
if [ -e ${DEPLOY_DIR_IMAGE}/MLO ]; then
	cp ${DEPLOY_DIR_IMAGE}/MLO ${QUINCE_DEPLOY_DIR}/boot
fi
if [ -e ${DEPLOY_DIR_IMAGE}/u-boot.img ]; then
	cp ${DEPLOY_DIR_IMAGE}/u-boot.img ${QUINCE_DEPLOY_DIR}/boot
fi
if [ -e ${DEPLOY_DIR_IMAGE}/zImage ]; then
	cp ${DEPLOY_DIR_IMAGE}/zImage ${QUINCE_DEPLOY_DIR}/boot
fi


#mkdir -p ${QUINCE_DEPLOY_DIR}/boot/extlinux
#if [ -e ${DEPLOY_DIR_IMAGE}/extlinux.conf ]; then
#	cp ${DEPLOY_DIR_IMAGE}/extlinux.conf ${QUINCE_DEPLOY_DIR}/boot/extlinux
#fi
